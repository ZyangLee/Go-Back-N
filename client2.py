...
class CLIENT:
    MY_ADDR = ('localhost', 43057)
    YOUR_ADDR = ('localhost', 43056)
    ...
    def receiver(self, filename):
        print('client2\'s receiver is running')
        # 将packet中信息保存到文件中
        try:
...
if __name__ == '__main__':
    client2 = CLIENT()
    client_receiver = threading.Thread(target=client2.receiver, args=('./copy1.txt',))
    client_receiver.start()
    client_sender = threading.Thread(target=client2.sender, args=('./test2.txt',))
    client_sender.start()
